...
from sensor_msgs.msg import Image as imggg
...
trained_model = "./src/yolact/yolact/weights/yolact_resnet50_54_800000.pth"
top_k = 5
...
image = 'input_image.png:output_image.png' # A path to an image to use for display.'
...
score_threshold = 0.9 # Detections with a score under this threshold will not be considered. This currently only works in display mode.
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Copy'):
        ...
        print(classes,scores,boxes) # results inference detections
...
def evalimage(net:Yolact, img, save_path:str=None):
    # cv2.imread(path) is the numpy array of the image
    frame = torch.from_numpy(img).cuda().float()
    batch = FastBaseTransform()(frame.unsqueeze(0))
    preds = net(batch)
    try:
        classes,scores,boxes = prep_display(preds, frame, None, None, undo_transform=False)
        val = (classes,scores,boxes)
    except ValueError:
        val = "NULL"
    
    # if save_path is None:
    #     img_numpy = img_numpy[:, :, (2, 1, 0)]
    return val  # classes, score, boxes
    # if save_path is None:
...
def evaluate(net:Yolact, img, train_mode=False):
...
class Evaluator(Node):
    def __init__(self):
        ...
        with torch.no_grad():
            ...
            if resume and not display:
                with open(ap_data_file, 'rb') as f:
                    ap_data = pickle.load(f)
                calc_map(ap_data)
                exit()

            if image is None and video is None and images is None:
